...
class VisualGridHuntGame:
    ...
    def execute_action(self, action: str):
        ...
        # Rule 1: Suck food at current location
        if action == "Suck":
        ...
        # Rule 2: Turn left (change facing direction only)
        if action == "Left":
        ...
        # Rule 3: Move forward in the current facing direction
...
class SimpleReflexAgent:
    """A simple reflex agent. It only looks at the current percept and applies IF-THEN rules. It has NO memory."""
...
class ModelBasedAgent:
    ...
    def sense_and_act(self, percept):
        ...
        # Current state based only on what the agent can perceive
        current_state = (
            percept["wall_ahead"],
            percept["food_here"]
        )

        # Check memory BEFORE adding current state
        already_visited = current_state in self.visited_cells
        ...
        # IF wall_ahead AND state already visited
        # THEN turn right
        elif percept["wall_ahead"] and already_visited:
            action = "Right"
...
class GridGameGUI:
    ...
    def __init__(self, root, width=10, height=10, num_food=12, num_opponents=2, walls=None):
        ...
        self.label = tk.Label(root, text="Score: 0 | Steps: 0 | Memory: 0", font=("Arial", 14))
